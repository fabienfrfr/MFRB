...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{Conceptual diagram of the model: firms in competitive pressure (\(\gamma\)), automation costs (\(k\)), and equilibrium \(\bar{a}^*\) system.}
    \label{fig:schema}
\end{figure}
...
In a simplified representation, the strategies are \textbf{Cooperation} (\(C: a \rightarrow 0\)), interpreted as maintaining a labor-intensive state, and \textbf{Defection} (\(D: a \rightarrow 1\)), interpreted as full automation. Each payoff is written as \((\Pi_i, W)\), where \(\Pi_i\) denotes firm-level profit and \(W\) denotes social welfare.
...
\begin{table}[h]
    ...
    \begin{tabular}{c c c}
        ...
        \cmidrule(lr){2-3}
        \textbf{Firm $i$} & Cooperation (\(C: a_j\rightarrow 0\)) & Defection (\(D: a_j \rightarrow 1\)) \\
        \midrule
        Cooperation (\(C: a_i\rightarrow 0\)) & \((0, W_{\text{Max}})\) & \((0, W_{\text{Low}})\) \\
        Defection (\(D: a_i\rightarrow 1\)) & \((\gamma + \beta - k, W_{\text{Low}})\) & \((\beta - k, W_{\text{Trap}})\) \\
        \bottomrule
    \end{tabular}
    \vspace{0.5em}
    \begin{flushleft}
...
We evaluate the model across a grid of competitive pressure parameters (\(\gamma \in \{0.5, 1.0, 2.0, 3.0\}\)), automation costs (\(k \in \{0.2, 0.8, 1.4, 2.0\}\)), and baseline automation gain \(\beta  \in \{0.2, 0.8, 1.4, 2.0\}\). Each \((\gamma, k)\) combination is simulated 10 times to account for stochasticity. Results, summarized in Figure~\ref{fig:analytical_results}-~\ref{fig:simulation_results}, highlight two key patterns: (1) rapid convergence to high automation levels \(\bar{a}^*\) when \(\gamma\) dominates \(k\), and (2) cost sensitivity, where \(k \geq 1.4\) significantly suppresses adoption even under strong competitive pressure. Time-series dynamics confirm equilibrium is reached within 500 iterations for most cases, validating the theoretical framework.
...
\subsection{Socio-Economic Implications: The End of Work and Money}

The simulation results suggest that the evolution of labor and monetary systems depends on the interaction between competitive dynamics and policy frameworks. First, the data indicates that high automation levels are a structural outcome of competitive pressure rather than a mere technological inevitability. Under high competitive advantage (\(\gamma \geq 2.0\)), firms automate extensively (\(\bar{a}^* \geq 0.87\)), suggesting that in sectors where AGI achieves high task-efficiency, human labor displacement may become a necessary condition for firm survival within a non-cooperative framework.

Second, these dynamics imply a potential shift in the role of monetary exchange. If automation levels approach \(\bar{a}^* \approx 1\), the resulting decline in wage-based income may reduce aggregate demand, potentially destabilizing traditional monetary circulation. \textbf{This creates a systemic paradox where individual firm rationality leads to the collective erosion of the consumer demand required for market survival.} Indeed, the velocity of money ($V$)—central to the equation $MV=PT$ as defined by Fisher~\cite{fisher1911purchasing}—might drop to zero if no human transactions $T$ occur, rendering the monetary system irrelevant\footnote{Where $M$ is the money supply, $V$ is the velocity of money, $P$ is the price level, and $T$ is the volume of transactions.}.. Consequently, maintaining monetary stability might require a transition toward alternative distribution mechanisms, such as Universal Basic Income or public investment, to decouple consumption from labor \cite{nayebi2025ai}.

Finally, these findings highlight that the economic consequences of AGI are mediated by institutional parameters. Since automation trajectories are sensitive to implementation costs (\(k\)) and competitive intensity (\(\gamma\)), the distribution of productivity gains remains a function of the prevailing regulatory and political environment \cite{korinek2024economic}. This underscores the role of policy instruments in mitigating "automation traps" and determining the long-term viability of the economic system.
