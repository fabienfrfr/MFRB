...
\begin{abstract}
The potential for Artificial General Intelligence~(\textsc{agi}) to automate a wide range of economically relevant tasks raises critical questions about the future of labor markets.
...
Simulations reveal that automation adoption patterns vary significantly with competitive pressure and cost structures:
high competitive intensity drives near-complete automation in some sectors, while elevated implementation costs preserve partial labor substitution in others.
These dynamics suggest that unregulated competitive interactions may lead to socially suboptimal automation levels, motivating the exploration of policy instruments such as automation taxation or universal basic income mechanisms funded by \textsc{agi}-generated rents.

The study contributes to the literature by isolating competitive incentives as a key driver of automation, distinct from purely technological or macroeconomic determinants, and underscores the role of institutional design in shaping the economic impact of \textsc{agi}.
\end{abstract}
...
Standard microeconomic theory posits that firms maximize profits \(\Pi_i = R_i - C_i\), where \(\Pi_i\) denotes the profit of firm \(i\), \(R_i\) its revenue, and \(C_i\) its costs. In constrained optimization settings, firms may instead maximize a utility function \(U_i(x_i)\) subject to resource constraints like : 
...
where \(U_i\) represents the firm's objective (e.g., profit, market share, or a combination of economic outcomes), and \(\omega\) is the total available resource budget \cite{varian1992microeconomic}. The Pareto optimum requires equal marginal utilities across agents:
...
\begin{figure}[ht]
    ...
    % Flèches entrantes vers f_i (ANGLES DROITS)
    % \draw[interaction_link, <-] (a1.south) -- ++(0,-0.7) -| (f2.south west)
    % node[pos=0.15, below, font=\footnotesize] {From $\bar{a}_{-i}$};
    % \draw[interaction_link, <-] (aN.south) -- ++(0,-0.7) -| (f2.south east)
    % node[pos=0.15, below, font=\footnotesize] {From $\bar{a}_{-i}$};
...
\[
\Pi_{i}(Q_i, a_i, Q_{-i}) = \underbrace{P(Q_i + Q_{-i}) \cdot Q_i}_{\text{Incomes}} - \underbrace{(c_0 - \delta a_i) Q_i - k a_i^2}_{\text{Costs}}
\]

% Hypotheses à reformuler pour clarifier le modèle
% Hypotheses 1 : Le travail est substituable par l'AGI et apporte la meme productivité
% Hypothèses 2 : Le travail humain et de l'AGI sont chacun un cout
% Hypothèses 3 : Le delta du cout apporte un avantage compétitif (prix de vente plus bas ou marge plus haute)

Component of Pi Origin (Modeling Hypothesis) \cite{acemoglu2018race,dasgupta1980uncertainty} Mathematical Link (Reduced Form) Term $\beta a_i$ (Absolute Productivity Gain) H1: Efficiency of AGI. The marginal cost of production $c$ is a decreasing and linear function of the automation $a_i$. $\Delta c \propto -a_i$ Captures the marginal gain (Revenue - Production Costs) which is independent of rivals' choices. Term $\gamma a_i (1 - \overline{a}_{-i})$ (Competitive Advantage) H2: Price Competition. Automation $a_i$ allows for a price decrease (or a better margin at constant price), resulting in a market share gain proportional to the automation gap with the average of rivals ($\overline{a}_{-i}$). It captures demand sensitivity to cost/price competitiveness, creating a transitory advantage. Term $-k a_i^2$ (Investment Cost) H3: Diminishing Returns on Investment. The total cost of implementing AGI increases more than proportionally ($k>0$) with the level of automation $a_i$. It anchors the equilibrium by preventing $a_i \to \infty$. It represents increasing integration difficulties. We can simplify the profit function by normalizing quantities and prices, leading to the following reduced form:
...
\[
\bar{a}_{\Pi}^* = \min\left(1, \frac{\gamma + \beta}{2k + \gamma}\right).
\]


\subsection{The Social Prisoner's Dilemma}

Our framework reveals a fundamental structural conflict, rendering the automation process a two-level Prisoner's Dilemma (PD) because its \cite{axelrod1981evolution}.

\subsubsection{Two-Level Objectives}
Firms, operating at the micro-level, independently maximize their net profit ($\Pi_i$). The resulting Nash Equilibrium, $\overline{a}_{\Pi}^*$, reflects the collectively self-destructive outcome of the competitive race. This equilibrium is compared against the Social Optimum, $\overline{a}_{W}^*$, which maximizes the aggregate Social Welfare ($W$) \cite{karabarbounis2014global, keynes1937general}.

\subsubsection{Social Welfare Function ($W$)}
Social welfare is defined as the balance between the aggregate economic gains from AGI efficiency ($G$) and the macroeconomic costs associated with salary-based demand collapse ($C$):
$$W(\overline{a}) = G(\overline{a}) - C(\overline{a})$$
The components are:
\begin{enumerate}
    \item \textbf{Efficiency Gain ($G$):} Directly proportional to the aggregate automation level, $\overline{a}$, capturing intrinsic productivity increases ($\beta$):
    $$G(\overline{a}) = \beta \overline{a}$$
    \item \textbf{Social Cost ($C$):} Non-linearly increasing with automation, reflecting the accelerating risk of demand collapse as human employment (proportional to $1-\overline{a}$) declines. We model this cost using a power function with $\lambda > 1$:
    $$C(\overline{a}) = C_{0} \cdot \overline{a}^{\lambda}$$
\end{enumerate}
The full Social Welfare function is:
$$W(\overline{a}) = \beta \overline{a} - C_{0} \overline{a}^{\lambda}$$

Similarity, the first-order condition for welfare maximization is solving for \(a_i\) yields the \textbf{reaction function}:

\[
\frac{\partial W(\overline{a})}{\partial \overline{a}} = \beta - \lambda C_{0}\overline{a}^{\lambda-1} = 0
\]
\[
\overline{a}_{W}^{*} = \left( \frac{\beta}{\lambda C_{0}} \right)^{\frac{1}{\lambda-1}}
\]

\subsubsection{The Dilemma}
The dilemma exists when the equilibrium chosen by firms ($\overline{a}_{\Pi}^*$) significantly exceeds the socially optimal level ($\overline{a}_{W}^*$).
The Nash Equilibrium $\overline{a}_{\Pi}^*$ is the outcome of profit maximization:
$$\overline{a}_{\Pi}^* \in \arg\max \Pi_i(a_i, \overline{a}_{-i})$$
The Social Optimum $\overline{a}_{W}^*$ is derived from maximizing welfare:
$$\overline{a}_{W}^* = \arg\max W(\overline{a})$$

If the parameters lead to a high $\overline{a}_{\Pi}^*$ (e.g., $\overline{a}_{\Pi}^* \approx 1$) where $C_{\text{social}}$ dominates $G_{\text{effic}}$, then:
$$\overline{a}_{\Pi}^* \gg \overline{a}_{W}^*$$
This divergence proves that the non-cooperative pursuit of micro-profit leads to a state of \textbf{socially sub-optimal welfare} (low $W$) and validates the analogy of the "Automation Trap" as a two-level Prisoner's Dilemma.


\subsection{The Automation Trap}
The iterative best-response dynamics converge to the symmetric equilibrium under standard conditions. The process of AGI automation is modeled as a two-level game, where the micro-level profit maximization drives a macro-level social sub-optimality. To formally capture this "Automation Trap," we present a dual-payoff matrix where each cell represents the outcome for both the individual firm and aggregate society.
...
The strategies are \textbf{Cooperation ($C: a=0$)}, defined as maintaining the current labor-intensive state (optimal for social stability), and \textbf{Defection ($D: a=1$)}, defined as maximum automation (optimal for short-term competitive gain). Each payoff takes the form: $(\Pi_i \text{ (Firm Profit)}, W \text{ (Social Welfare)})$.
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{c c c}
            ...
            \cmidrule(lr){2-3}
            \textbf{Firm $i$} & Cooperation ($C: a_j=0$) & Defection ($D: a_j=1$) \\
            \midrule
            Cooperation ($C: a_i=0$) & $(0, W_{\text{Max}})$ & $(0, W_{\text{Low}})$ \\
            Defection ($D: a_i=1$) & $(\gamma + \beta - k, W_{\text{Low}})$ & $(\beta - k, W_{\text{Trap}})$ \\
            \bottomrule
        \end{tabular}
        \vspace{0.5em}
        \begin{flushleft}
        \footnotesize
        \textbf{Key:} $\Pi_i$ is the firm's net profit (Sec. 2). $W$ is aggregate Social Welfare (Sec. 3), defined as $W(\overline{a}) = \beta \overline{a} - C_{0} \overline{a}^{\lambda}$.
        \textbf{The Dilemma:} The Nash Equilibrium is (Defection, Defection), driven by the firm's strong incentive: $\Pi(D, C) > \Pi(C, C)$ and $\Pi(D, D) > \Pi(C, D)$ (assuming $\beta > k$). However, this equilibrium leads to $W_{\text{Trap}}$, where $W_{\text{Max}} > W_{\text{Trap}}$, proving the social sub-optimality.
        \end{flushleft}
    \end{table}
