...
Our framework reveals a fundamental structural conflict, rendering the automation process a two-level Prisoner's Dilemma (PD) because its \cite{axelrod1984evolution}.

\subsubsection{Two-Level Objectives}
Firms, operating at the micro-level, independently maximize their net profit ($\Pi_i$). The resulting Nash Equilibrium, $\overline{a}_{\text{Nash}}^*$, reflects the collectively self-destructive outcome of the competitive race. This equilibrium is compared against the Social Optimum, $\overline{a}_{W}^*$, which maximizes the aggregate Social Welfare ($W$) \cite{karabarbounis2014global, keynes1937general}.

\subsubsection{Social Welfare Function ($W$)}
Social welfare is defined as the balance between the aggregate economic gains from AGI efficiency ($G_{\text{effic}}$) and the macroeconomic costs associated with salary-based demand collapse ($C_{\text{social}}$):
$$W(\overline{a}) = G_{\text{effic}}(\overline{a}) - C_{\text{social}}(\overline{a})$$
The components are:
\begin{enumerate}
    \item \textbf{Efficiency Gain ($G_{\text{effic}}$):} Directly proportional to the aggregate automation level, $\overline{a}$, capturing intrinsic productivity increases ($\beta$):
    $$G_{\text{effic}}(\overline{a}) = \beta \overline{a}$$
    \item \textbf{Social Cost ($C_{\text{social}}$):} Non-linearly increasing with automation, reflecting the accelerating risk of demand collapse as human employment (proportional to $1-\overline{a}$) declines. We model this cost using a power function with $\lambda > 1$:
    $$C_{\text{social}}(\overline{a}) = C_{0} \cdot \overline{a}^{\lambda}$$
\end{enumerate}
The full Social Welfare function is:
$$W(\overline{a}) = \beta \overline{a} - C_{0} \overline{a}^{\lambda}$$

\subsubsection{The Dilemma}
The dilemma exists when the equilibrium chosen by firms ($\overline{a}_{\text{Nash}}^*$) significantly exceeds the socially optimal level ($\overline{a}_{W}^*$).
The Nash Equilibrium $\overline{a}_{\text{Nash}}^*$ is the outcome of profit maximization:
$$\overline{a}_{\text{Nash}}^* \in \arg\max \Pi_i(a_i, \overline{a}_{-i})$$
The Social Optimum $\overline{a}_{W}^*$ is derived from maximizing welfare:
$$\overline{a}_{W}^* = \arg\max W(\overline{a})$$

If the parameters lead to a high $\overline{a}_{\text{Nash}}^*$ (e.g., $\overline{a}_{\text{Nash}}^* \approx 1$) where $C_{\text{social}}$ dominates $G_{\text{effic}}$, then:
$$\overline{a}_{\text{Nash}}^* \gg \overline{a}_{W}^*$$
This divergence proves that the non-cooperative pursuit of micro-profit leads to a state of **socially sub-optimal welfare** (low $W$) and validates the analogy of the "Automation Trap" as a two-level Prisoner's Dilemma.
